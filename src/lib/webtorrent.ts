...
export const MAGNET_TRACKERS = 'udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337&tr=udp%3A%2F%2Fexplodie.org%3A6969&tr=udp%3A%2F%2Ftracker.empire-js.us%3A1337&tr=wss%3A%2F%2Ftracker.btorrent.xyz&tr=wss%3A%2F%2Ftracker.openwebtorrent.com&tr=wss%3A%2F%2Ftracker.webtorrent.dev';
...
const setupTorrentEvents = (torrent: Torrent, eshareId: string, selfDestruct: boolean) => {
    // Initial stats
...
export const resumeSeeding = async () => {
    ...
    for (const link of links) {
        ...
        const metadataBlob = new Blob([JSON.stringify(link)], { type: 'application/json' });
        const metadataFile = new File([metadataBlob], METADATA_FILENAME, { type: 'application/json' });
        const contentFile = new File([content.blob], CONTENT_FILENAME, { type: content.blob.type });
...
export const seedFiles = async (metadata: File, content: File, eshareId: string, selfDestruct: boolean = false): Promise<SeedResult> => {
...
type TorrentEvent = "infoHash" | "metadata" | "ready" | "done" | "warning" | "error" | "download" | "upload" | "wire" | "noPeers";
...
export const fetchFile = (torrent: Torrent, { on }: ToastOptions): Promise<TorrentFiles> => {
                ...
                Promise.all([
                    new Promise<File>((resolve, reject) => {
                        metadata.getBuffer((err, buffer) => {
                            if (err) reject(err);
                            else resolve(new File([buffer!], METADATA_FILENAME, { type: 'application/json' }));
                        });
                    }),
                    new Promise<File>((resolve, reject) => {
                        content.getBuffer((err, buffer) => {
                            if (err) reject(err);
                            else resolve(new File([buffer!], CONTENT_FILENAME, { type: 'application/octet-stream' }));
                        });
                    })
